...
class Trainer(object):
    def __init__(self, args):
        ...
        self.first_device = self.args.gpu_ids[0]
        ...
        if args.use_balanced_weights:
            ...
            weight = None
        self.criterion = SegmentationLosses(weight=weight, device=self.first_device).build_loss(mode=args.loss_type)
        ...
        self.scheduler = LR_Scheduler(args.lr_scheduler, args.lr,
                                            args.epochs, len(self.train_loader), start_epoch=args.lr_start)

        # Using cuda
        if args.cuda:
            self.model = torch.nn.DataParallel(self.model, device_ids=self.args.gpu_ids)
            patch_replication_callback(self.model)
            self.model = self.model.to(self.first_device)
    ...
    def training(self, epoch):
        ...
        for i, sample in enumerate(tbar):
            ...
            if self.args.cuda:
                image, target = image.to(self.first_device), target.to(self.first_device)
            ...
            if self.args.densecrfloss ==0:
                ...
                if self.args.cuda:
                    densecrfloss = densecrfloss.to(self.first_device)
...
def main():
    ...
    parser.add_argument('--lr-start', type=int, default=0,
                        help='epoch to start lr scheduler from (for resuming)')
