...
class DenseCRFLossFunction(Function):
    ...
    def forward(ctx, images, segmentations, sigma_rgb, sigma_xy, ROIs):
        ...
        segmentations = torch.mul(segmentations.to(device), ROIs.to(device))
        ctx.ROIs = ROIs
    ...
    def backward(ctx, grad_output):
        ...
        grad_segmentation=grad_segmentation.to(device)
        grad_segmentation = torch.mul(grad_segmentation, ctx.ROIs.to(device))
